...
const {
  ...
  nextPage,
  prevPage
} = searchBySlice.actions;
const getSearchSelector = state => state.search;
...
const swApi = createApi({
  ...
  endpoints: builder => ({
    getCharacters: builder.query({
      query: id => `people/?page=${id}`
    }),
    searchPeople: builder.query({
      query: searchInput => `people/?search=${searchInput}`
    }),
    getSpecies: builder.query({
      query: id => `species/?page=${id}`
    }),
    searchSpecies: builder.query({
      query: searchInput => `species/?search=${searchInput}`
    })
  })
});
...
export { changeSort, getSearchSelector, nextPage, prevPage, store, swApi, useGetCharactersQuery, useGetSpeciesQuery, useSearchPeopleQuery, useSearchSpeciesQuery };
